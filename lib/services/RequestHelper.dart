    ...
    try {
      ...
      if (201 == response.statusCode) {
        ...
        if (parsed['message'] != null)
          Toast.show(parsed['message'], duration: 2);
        print(parsed['message']);
        return response;
      } else if (404 == response.statusCode) {
        final parsed = json.decode(response.body);
        if (parsed["message"] != null)
          Toast.show(parsed["message"][0],duration: 2);
        Navigator.pop(context);
        return null;
      } else if (400 == response.statusCode) {
        final parsed = json.decode(response.body).cast<String, dynamic>();
        if (parsed["username"] != null)
          Toast.show(parsed["username"][0], duration: 2);
        else if (parsed["email"] != null)
          Toast.show(parsed["email"][0],  duration: 2);
        else if (parsed['password'] != null)
          Toast.show(parsed['password'][0],  duration: 2);
        return null;
      } else {
        return null;
      }
